...
const data =
  props.type === BookGridType.VocabSupplement
    ? (rawData as YoutubeData).supplementVocabPlaylists
    : (rawData as YoutubeData).bookAudioPlaylists;
...
const bookItems = computed(() =>
  (() => {
    ...
    if (props.type === BookGridType.Manipulatives) {
      ...
      return data.filter(
        (a) =>
          includes.findIndex((bookName) =>
            a.playlistTitle.includes(bookName)
          ) >= 0
      );
    }
    return data;
